<?php

declare(strict_types=1);

namespace App\Entity;

use App\Repository\EstncGruposPermissoesRepository;
use Doctrine\ORM\Mapping as ORM;

#[ORM\Entity(repositoryClass: EstncGruposPermissoesRepository::class)]
#[ORM\Table(
    name: 'estnc_grupos_permissoes',
    options: ['charset' => 'latin1', 'collation' => 'latin1_swedish_ci']
)]
#[ORM\Index(name: 'IX_CD_GRUPO', columns: ['CD_GRUPO'])]
#[ORM\Index(name: 'IX_CD_GRUPO_ACAO', columns: ['CD_GRUPO_ACAO'])]
class EstncGruposPermissoes
{
    #[ORM\Id]
    #[ORM\Column(name: 'CD_GRUPO', type: 'integer', options: ['unsigned' => true])]
    private ?int $cdGrupo = null;

    #[ORM\Id]
    #[ORM\Column(name: 'CD_GRUPO_ACAO', type: 'integer', options: ['unsigned' => true])]
    private ?int $cdGrupoAcao = null;

    public function __construct(
        ?int $cdGrupo = null,
        ?int $cdGrupoAcao = null
    ) {
        $this->cdGrupo = $cdGrupo;
        $this->cdGrupoAcao = $cdGrupoAcao;
    }

    public function getCdGrupo(): ?int
    {
        return $this->cdGrupo;
    }

    public function setCdGrupo(?int $cdGrupo): self
    {
        $this->cdGrupo = $cdGrupo;
        return $this;
    }

    public function getCdGrupoAcao(): ?int
    {
        return $this->cdGrupoAcao;
    }

    public function setCdGrupoAcao(?int $cdGrupoAcao): self
    {
        $this->cdGrupoAcao = $cdGrupoAcao;
        return $this;
    }
}
